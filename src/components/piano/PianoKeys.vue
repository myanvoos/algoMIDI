...
const whiteKeys = computed(() => fullKeyboard.filter(note => !note.name.includes('#')));
const blackKeys = computed(() => fullKeyboard.filter(note => note.name.includes('#')));

const isNotePressed = (note: Note): boolean => {
  return Array.from(props.pressedKeys).some(pressedNote => 
    pressedNote.name === note.name
  );
}

const getPressedNote = (note: Note): Note | undefined => {
  return Array.from(props.pressedKeys).find(pressedNote => 
    pressedNote.name === note.name
  );
}
